...
# basic stopwords (not important for us). These are ONLY USED IF stopword removal is turned on.
...
# regex patterns
# These help normalize common risky message patterns into stable tokens.
...
# These subtype rules are only simple keyword rules.
...
# These categories are more general. If they overlap with more specific categories, we usually prefer the specific one.
...
# Bigger value = More specific / more important when overlaps happen.
...
# Compile the config regex once so row-by-row matching stays fast and readable.
...
def clean_message_text(message_text: str, remove_stopwords: bool = False) -> str:
    # Normalize message text Main steps: normalize unicode, replace urls/emails/phones with stable tokens, lowercase, remove most punctuation, adn normalize spaces.
...
def create_unigram_bigram_ready_text(cleaned_message_text: str) -> str:
     # Build one text field that includes original tokens and adjacent token bigrams joined with underscores
...
def extract_annotation_candidates_from_message_text(message_text: str) -> list[dict[str, Any]]:
    # Extract all raw regex matches from the original message text. This happens before overlap cleanup.
...
def annotations_overlap(
    ...
    # Check if two annotation spans overlap at all.
    return (
...
def annotation_contains_other(
    ...
    # Check if one annotation span fully contains another.
    return (
...
def annotations_overlap_heavily(
    ...
    # Decide if overlap is strong enough to matter. We use 80% of the shorter span as the threshold.
...
def annotations_are_distinct_enough(
    ...
    # Decide whether two overlapping annotations should both stay. We keep both if they add clearly different value.
...
def choose_preferred_overlapping_annotation(
    ...
    # Pick the better annotation when two overlapping annotations  should not both stay.
    first_annotation_is_general = annotation_category_is_general(first_annotation)
    second_annotation_is_general = annotation_category_is_general(second_annotation)
...
def merge_overlapping_annotations(annotation_list: list[dict[str, Any]]) -> list[dict[str, Any]]:
    # Clean overlap conflicts so the final annotation list stays readable. Sorting like this: earlier spans first, longer phrases first, an dmore specific categories first.
...
def assign_optional_scam_subtype(cleaned_message_text: str, label: int) -> str:
    # Assign a simple scam subtype using keyword rules. Safe rows always get 'none'.
...
def create_basic_text_features(message_text: str, cleaned_message_text: str) -> dict[str, int]:
    # Create a few lightweight numeric features. Just helper features!
...
def build_recommended_vectorizer_configuration() -> dict[str, Any]:
    # Return the recommended traditional vectorizer config.
...
def prepare_dataset_for_machine_learning(
    ...
    # Turn the raw collected rows into a processed dataset. Main work is to filter empty/too short rows, clean message text, add unigram + bigram ready text, add optional scam subtype, add suspicious phrase annotations, add simple text features, and remove conflicting duplicate texts.
    ...
    for row in raw_dataset.to_dict(orient="records"):
        ...
        # We run phrase extraction on all rows.
        # Safe rows can still contain risky-looking language.
    ...
    # If the same cleaned text appears with both labels, keeping both would teach the model contradictory information.
    ...
    # Drop exact duplicates affer cleaning, keeping the first.
